...
def HR_network(X, i):
    ...
    ISyn = (sey + seye) * (x - VsynE) + siy * (x - Ecl)
...
sGsynExt = 0.2
VsynE = 0;
Ecl = 0 # reversal potential  -2.0 -1.5 -1 -0.5 0
tau1E = 3;
tau2E = 1
tau1I = 4;
tau2I = 1
...
if isim % threads == rank:
    ...
    for i in range(nsteps):
        ...
        # Ecl_t[i, :] = E_cl_now
        x, y, z, sex, sey, six, siy, sexe, seye = X  # agregué variable 's'
        ISyn_t[i, :] = (X[4] + X[8]) * (X[0] - VsynE) + X[6] * (X[0] - Ecl)
        ISyn_It[i, :] =X[6] * (X[0] - Ecl)
...
np.savetxt(f"Ecl{Ecl}V.txt", V_w[:, idx1], fmt="%.6f")
# np.savetxt(f"{jee}Ecl.txt", Ecl_w[:, idx1], fmt="%.6f")
np.savetxt(f"Ecl{Ecl}Icl.txt", Icl_w[:, idx1], fmt="%.6f")
np.savetxt(f"Ecl{Ecl}Isyn.txt", ISyn_w[:, idx1], fmt="%.6f")
np.savetxt(f"Ecl{Ecl}siy.txt", siy_w[:, idx1], fmt="%.6f")
# np.savetxt(f"{jee}x_Ecl.txt", x_Ecl_w[:, idx1], fmt="%.6f")

# np.savetxt(f"Ecl{Ecl}mean_siy_E.txt", mean_siy_E[:], fmt="%.6f")
# np.savetxt(f"{jee}mean_Icl_E.txt", mean_Icl_E[:], fmt="%.6f")
# np.savetxt(f"{jee}mean_x_Ecl_E.txt", mean_xEcl_E[:], fmt="%.6f")
# np.savetxt(f"{jee}mean_Ecl_E.txt", mean_Ecl_E[:], fmt="%.6f")



# np.savetxt(f"scaling{scaling}Ecl.txt", Ecl_w[:, idx1], fmt="%.6f")
spikeToFile(spikes, f"Ecl{Ecl}spikes.txt")

plt.figure(figsize=(10, 6))
plt.plot(Time, V_w[:, idx1], label=f"x neuron {idx1+1}")
plt.legend()
plt.title("HR-V")
plt.savefig(f"Ecl{Ecl}V.png", dpi=300)